...
public class ViewMarkReport extends JInternalFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (ae.getSource() == btnView) 
        {
            ...
            if (qualification.equalsIgnoreCase("PLUSTWO")) {
                tablenme = "PLUSTWO";
            } else {
                tablenme = "SSLC";
            }
            ...
            try {
                ...
                rs = st.executeQuery("SELECT count(*) from " + tablenme + " Where RollNo=" + Integer.parseInt(rollno));
                if (rs.next()) {
                    //rs.close();
                    String sql = "SELECT A.SubjectName, A.SubjectCode, A.Internal, A.Theory, A.Practical, A.Total, B.MaxMark FROM " +
                            tablenme + " AS A, Subjects AS B WHERE A.Subjectcode=B.Subjectcode and RollNo=" + Integer.parseInt(rollno)+
                            " and B.Type='" + type + "'";
              ResultSet rs1 = st.executeQuery(sql);
        System.out.println("table name is "+tablenme+"----"+"roll no is"+rollno+"---"+"type is"+type);
                    while (rs1.next()) {
                        subname = rs1.getString(1);
                        subcode = rs1.getString(2);
                        internal = rs1.getInt(3);
                        theory = rs1.getInt(4);
                        practical = rs1.getInt(5);
                        total = rs1.getInt(6);
                        maxmark = rs1.getInt(7);
                        grandtotal = grandtotal + total;
                        if (theory < 40) {
                            result = "FAILED";
                        } else {
                            sql = "SELECT Practical from Subjects WHERE Subjectcode='" + subcode + "'";
